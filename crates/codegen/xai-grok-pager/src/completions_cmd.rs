...
pub fn run(shell: Shell) {
    // Ensure the script always uses the public "abxglia-grok" name (matches historical
    // behavior and what the installers + docs expect).
    let mut cmd = PagerArgs::command().name("abxglia-grok");
    if shell != Shell::Zsh {
        generate(shell, &mut cmd, "abxglia-grok", &mut std::io::stdout());
        return;
    }
    // zsh needs post-processing (see fix_zsh_root_prompt_positional).
    let mut buf = Vec::new();
    generate(shell, &mut cmd, "abxglia-grok", &mut buf);
...
fn fix_zsh_root_prompt_positional(script: &str) -> String {
    ...
    for (from, to) in [
        ...
        (
            r#"curcontext="${curcontext%:*:*}:abxglia-grok-command-$line[2]:""#,
            r#"curcontext="${curcontext%:*:*}:abxglia-grok-command-$line[1]:""#,
        ),
...
mod tests {
    ...
    fn zsh_script() -> String {
        let mut cmd = PagerArgs::command().name("abxglia-grok");
        let mut buf = Vec::new();
        generate(Shell::Zsh, &mut cmd, "abxglia-grok", &mut buf);
